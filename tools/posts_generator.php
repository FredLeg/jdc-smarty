...
for ($i = 0; $i < 50; $i++) {
	...
	$content = $contents[$i];
	$first_point_space = strpos($content, ' ');

	$author = substr($content, 0, $first_point_space);
	$content = substr($content, $first_point_space + 1);
...
foreach($posts as $key => $article) {
	...
	$query = $db->prepare('INSERT INTO posts (author, content, creation_date) VALUES (:author, :content, :creation_date)');
	$query->bindValue('creation_date', $article['creation_date']);
...
foreach($posts as $key => $article) {
	...
	echo $result.'|'.$creation_date.'|'.$content.'|'.$author.'<br>';
}